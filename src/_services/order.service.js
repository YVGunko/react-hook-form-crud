...
import { fetchWrapper, isString, isObjectEmpty } from '@/_helpers';
...
function getAll(dateFrom, dateTill, isUser, customer_id, division_code) {
  ...
  if (token && !isObjectEmpty(token.filial)) { queryParams = queryParams.concat(isString(queryParams) ? `&filial_id=${token.filial_id}` : `?filial_id=${token.filial_id}`); }
